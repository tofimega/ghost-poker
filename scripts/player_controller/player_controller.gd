...
func find_odds()->float:
	...
	rt/=(POWER_ADD_CRANK+POWER_ADD_HRANK)*mult_hsize*mult_players*MAX_VAR
	rt=ease(rt, -((1.0/(PokerEngine.current_turn+TIME_OFFSET)+0.3)*POWER_ADD_TIME))
