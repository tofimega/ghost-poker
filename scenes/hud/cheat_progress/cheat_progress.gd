...
func _process(delta: float) -> void:
	if get_tree().get_frame()%3==0:
		if texture_progress_bar.value == texture_progress_bar.max_value and \
		enabled and \
		texture_progress_bar.tint_progress==Color.GREEN: 
			texture_progress_bar.tint_progress=Color.LIGHT_GREEN
